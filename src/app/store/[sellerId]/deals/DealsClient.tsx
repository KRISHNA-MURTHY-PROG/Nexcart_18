...
export function DealsClient({ products, sellerId, storeName, color, scrollingDesign }: Props) {
      ...
      <div className="sticky top-0 z-10 flex flex-col gap-2 px-4 py-3 backdrop-blur-sm" style={{ background: `${color}ee` }}>
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2 flex-1">
            <Tag className="h-5 w-5 shrink-0 text-white" />
            <span className="font-black text-[16px] text-white">Deals of the Day</span>
          </div>
        ...
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center gap-3 py-24 text-center">
            ...
            <p className="font-bold text-foreground">
              {search ? `No results for "${search}"` : "No deal products right now"}
            </p>
            {search ? (
