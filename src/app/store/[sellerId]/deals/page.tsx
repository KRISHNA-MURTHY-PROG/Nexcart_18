...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return { title: `Deals of the Day — ${seller?.storeName ?? "Store"} | NexCart` };
}

export default async function DealsOfferPage({ params }: Props) {
  ...
  const dealOffers = await db.sellerOffer.findMany({
    where: { sellerId: seller.id, offerType: "DEALS_OF_THE_DAY", isActive: true },
    select: { linkedProductId: true, selectedProductIds: true },
  });

  const productIds = new Set<string>();
  for (const offer of dealOffers) {
    if (offer.linkedProductId) productIds.add(offer.linkedProductId);
    if (offer.selectedProductIds) {
      try {
        const ids = JSON.parse(offer.selectedProductIds as string) as string[];
  ...
  const products = productIds.size > 0
    ? await db.product.findMany({
        where: { id: { in: [...productIds] }, sellerId: seller.id, isActive: true, stock: { gt: 0 } },
        include: {
          category: true,
